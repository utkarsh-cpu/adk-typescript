...
export abstract class BaseAgent {
   ...
   * @yields {Event} The events generated by the agent.
   */
  protected abstract _runAsyncImpl(
   ...
   * @yields {Event} The events generated by the agent.
   */
  protected abstract _runLiveImpl(
    ctx: InvocationContext
  ): AsyncGenerator<Event>;
  ...
  protected _createInvocationContext(
    parentContext: InvocationContext
  ): InvocationContext {
   ...
   */
  private async __handleBeforeAgentCallback(
    ctx: InvocationContext
  ): Promise<Event | null> {
    ...
    let beforeAgentCallbackContent =
      await ctx.pluginManager.runBeforeAgentCallback(this, callbackContext);
   ...
   * @returns An event if callback provides content or changed state.
   */
  private async __handleAfterAgentCallback(
  ...
  private static __createKwargs(
    ...
    if (config.subAgents) {
      ...
      for (const _subAgentConfig of config.subAgents) {

...
export class LiveRequest{
    ...
    private validateNoExtraFields(data: any): void {
        const allowedFields = ['content', 'blob', 'activityStart', 'activityEnd', 'close'];
        const extraFields = Object.keys(data).filter(key => !allowedFields.includes(key));
        if (extraFields.length > 0) {
          throw new Error(`Extra fields not allowed in LiveRequest: ${extraFields.join(', ')}`);
        }
      }
...
export class LiveRequestQueue{
    ...
    sendActivityStart(): void {
        this.putNowait(new LiveRequest({ 

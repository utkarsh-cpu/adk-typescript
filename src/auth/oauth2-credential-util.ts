...
import { isOAuth2 } from './openapi_models';
...
const logger = {
  ...
  error: (message: string, error?: any) =>
    console.error(`[OAuth2Util] ${message}`, error),
};
...
function _createOAuth2Session(
  ...
  // Handle OAuth2 scheme
    ...
    authorizationEndpoint =
      authScheme.flows.authorizationCode.authorizationUrl || null;
...
// Export the experimental wrapped version
export const updateCredentialWithTokens = experimental()(
  _updateCredentialWithTokens
);
...
export function updateCredentialWithEnhancedTokens(
  ...
  authCredential.oauth2!.accessToken =
    tokens.accessToken || tokens.access_token || null;
  ...
  if (tokens.expiresIn || tokens.expires_in) {
    ...
    authCredential.oauth2!.expiresIn =
    ...
    if (!authCredential.oauth2!.expiresAt) {
      authCredential.oauth2!.expiresAt = new Date(
        Date.now() + authCredential.oauth2!.expiresIn! * 1000
      ).getTime();
    }
  }
...
export function parseScopeString(scopeString: string): string[] {
  return scopeString
    ? scopeString.split(' ').filter((scope) => scope.trim() !== '')
    : [];
}

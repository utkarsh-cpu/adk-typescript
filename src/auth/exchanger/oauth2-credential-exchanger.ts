...
import { AuthScheme, OAuthGrantType } from '../auth-schemes';
import { createOAuth2Session, updateCredentialWithTokens } from '../oauth2-credential-util';
import { BaseCredentialExchanger, CredentialExchangeError } from './base-credential-exchanger';

// Logger setup - using console for simplicity, you can replace with your preferred logging library
const logger = {
    warning: (message: string) => console.warn(`[OAuth2CredentialExchanger] ${message}`),
    debug: (message: string) => console.debug(`[OAuth2CredentialExchanger] ${message}`),
    error: (message: string, error?: any) => console.error(`[OAuth2CredentialExchanger] ${message}`, error),
};
...
export class OAuth2CredentialExchanger extends BaseCredentialExchanger {
    ...
    async exchange(
        ...
        const { client, tokenEndpoint } = await this.createOAuth2Session(authScheme, authCredential);
        ...
        try {
            ...
            if (authCredential.oauth2?.authCode) {
                ...
                throw new Error('No authorization code or auth response URI available for token exchange');
            }
     ...
     */
    private updateCredentialWithTokens(authCredential: AuthCredential, tokens: any): void {
        if (!authCredential.oauth2) {
            authCredential.oauth2 = {} as any;
        }

        authCredential.oauth2!.accessToken = tokens.accessToken || tokens.access_token;
        authCredential.oauth2!.refreshToken = tokens.refreshToken || tokens.refresh_token;

        // Set token expiration
        if (tokens.expiresAt) {
            authCredential.oauth2!.expiresAt = tokens.expiresAt;
        } else if (tokens.expires_in) {
            authCredential.oauth2!.expiresAt = new Date(Date.now() + tokens.expires_in * 1000).getTime();
        }
     ...
     */
    async refreshAccessToken(authCredential: AuthCredential, authScheme: AuthScheme): Promise<AuthCredential> {
        if (!authCredential.oauth2?.refreshToken) {
            throw new CredentialExchangeError('No refresh token available');
        }

        const { client } = await this.createOAuth2Session(authScheme, authCredential);
        if (!client) {
            throw new CredentialExchangeError('Could not create OAuth2 session for token refresh');
        }
        ...
        try {
            ...
            throw new CredentialExchangeError(`Failed to refresh access token: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
     ...
     */
    isTokenExpired(authCredential: AuthCredential, bufferMinutes: number = 5): boolean {
        ...
        const expirationTime: Date = (expiresAt as any) instanceof Date

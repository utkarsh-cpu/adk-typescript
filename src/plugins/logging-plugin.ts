...
export class LoggingPlugin extends BasePlugin {
    ...
    private _format_args(args: { [key: string]: any }, max_length: number = 300): string {
        if (!args) {
            return "{}";
        }
    ...
    async on_user_message_callback({
        ...
        this._log(
            `   Root Agent: ${invocation_context.agent && 'name' in invocation_context.agent
    ...
    async before_run_callback({
        ...
        this._log(`   Invocation ID: ${invocation_context.invocationId}`);
        this._log(
            `   Starting Agent: ${invocation_context.agent && 'name' in invocation_context.agent
                ? invocation_context.agent.name
                : 'Unknown'
            }`
        );
    ...
    async on_event_callback({
        ...
        if (event.longRunningToolIds && event.longRunningToolIds.size > 0) {
            this._log(`   Long Running Tools: ${Array.from(event.longRunningToolIds)}`);
        }
    ...
    async after_run_callback({
        ...
        this._log(`   Invocation ID: ${invocation_context.invocationId}`);
        this._log(
            `   Final Agent: ${invocation_context.agent && 'name' in invocation_context.agent
                ? invocation_context.agent.name
                : 'Unknown'
            }`
        );
    ...
    async before_agent_callback({
        ...
        if (callback_context._invocationContext?.branch) {  // Assuming access; adjust if private
    ...
    async before_model_callback({
        ...
        if (llm_request.config && llm_request.config.systemInstruction) {
            ...
            if (typeof llm_request.config.systemInstruction === 'string') {
                sys_instruction = llm_request.config.systemInstruction.substring(0, 200);
                ...
                sys_instruction = this._format_content(llm_request.config.systemInstruction, 200);
            } else {
    ...
    async after_model_callback({
        ...
        if (llm_response.errorCode) {
            ...
            if (llm_response.turnComplete !== undefined && llm_response.turnComplete !== null) {

...
export class CodeExecutorContext {
  ...
  addProcessedFileNames(fileNames: string[]): void {
    ...
    const existingFiles = this._context.get(
      _PROCESSED_FILE_NAMES_KEY
    ) as string[];
  ...
  getInputFiles(): File[] {
    ...
    return (this._sessionState[_INPUT_FILE_KEY] as any[]).map(
      (file) => new File(file.name, file.content, file.mimeType)
    );
  }
  ...
  getErrorCount(invocationId: string): number {
    ...
    const errorCounts = this._sessionState[_ERROR_COUNT_KEY] as Record<
      string,
      number
    >;
  ...
  updateCodeExecutionResult(
    ...
    const results = this._sessionState[_CODE_EXECUTION_RESULTS_KEY] as Record<
      string,
      any[]
    >;
  ...
  private _getCodeExecutorContext(sessionState: State): Map<string, any> {
    ...
    return new Map(
      Object.entries(sessionState[_CONTEXT_KEY] as Record<string, any>)
    );
  }
}
